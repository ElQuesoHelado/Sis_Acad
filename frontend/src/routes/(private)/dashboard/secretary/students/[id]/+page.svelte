<script lang="ts">
	...
	import { ChevronLeft, Loader2, AlertCircle, ChevronDown, BookOpen, User } from '@lucide/svelte';
	...
	$effect(() => {
		async function load() {
			...
			summaries = [];
			try {
				summaries = await secretaryService.getStudentDetails(userId, selectedSemester);
			} catch (err) {
		...
		<div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
			{#each summaries as summary}
				<Card class="flex flex-col overflow-hidden">
					<CardHeader class="bg-muted/20 border-b pb-4">
						<div class="flex items-start justify-between gap-4">
							<div>
								<CardTitle class="line-clamp-1 text-lg">{summary.courseName}</CardTitle>
								<div class="text-muted-foreground mt-1.5 flex items-center gap-2">
									<User class="h-4 w-4" />
							...
							<table class="w-full text-sm">
								<thead class="bg-muted/50 text-muted-foreground">
									<tr>
										...
										<th class="w-20 px-4 py-2 text-center font-medium">Peso</th>
										<th class="bg-primary/5 text-primary w-20 px-4 py-2 text-center font-medium"
											>Nota</th
										>
									</tr>
								</thead>
								<tbody class="divide-y">
									{#each buildGradeRows(summary) as row}
										<tr>
											...
											<td class="text-muted-foreground px-4 py-2 text-center">{row.weight}%</td>
											<td class="bg-primary/5 px-4 py-2 text-center font-bold">
												{#if row.score !== null}
