<script lang="ts">
	...
	$effect(() => {
		async function load() {
			...
			error = '';
			try {
				details = await adminService.getTeacherDetails(userId, selectedSemester);
			} catch (err) {
	...
	{:else if details}
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{#each details.groups as group}
							<div class="flex items-center justify-between p-3 border rounded-lg bg-card/50">
								<div>
									<p class="font-semibold text-sm line-clamp-1" title={group.courseName}>
										{group.courseName}
									</p>
		...
		{#if details.schedule.length > 0}
			<div class="relative overflow-auto rounded-lg border bg-card shadow-sm">
				...
				>
					<div class="sticky top-0 z-10 border-r border-b bg-muted/50 p-3 text-center font-semibold text-muted-foreground">Hora</div>
					{#each days as day}
						<div class="sticky top-0 z-10 border-b bg-muted/50 p-3 text-center font-semibold text-muted-foreground border-r last:border-r-0">
							{day}
						</div>
					{/each}

					{#each timeSlots as time, i}
						<div class="border-r p-3 text-right text-sm font-medium text-muted-foreground" style="grid-row-start: {i + 2};">
							{time}
						</div>
						{#each days as day, j}
							<div class="border-b border-r last:border-r-0" style="grid-row-start: {i + 2}; grid-column-start: {j + 2};"></div>
						{/each}
					{/each}
								...
								<HoverCard.Content class="w-80" side="right" align="start">
									<div class="space-y-4 p-2">
										...
										<div class="flex items-start gap-3">
											...
											<div>
												<h3 class="text-sm font-medium text-muted-foreground">Día y Hora</h3>
